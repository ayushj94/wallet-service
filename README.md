...
**A small, careful service that holds customer money and never loses a cent of it.**
...
>
> 1. A wallet must never go negative.
> 2. A retried request must never charge twice.
...
3. [Run it](#%EF%B8%8F-3-run-it)
4. [Try it out / API Contracts](#-4-try-it-out--api-contracts)
...
A logistics platform's customers prepay money into a wallet. Every order debits a fixed amount; topups credit it.

The wallet service is the source of truth for every dollar that moves:

- 🏦 it owns balances,
- 📜 records every movement to an append-only ledger,
- 🚫 enforces the never-negative rule, and
- 🪟 provides transparency to clients through `GET` endpoints (current balance + full transaction history).
...
> 💡 If you already have a Postgres instance you want to use, skip Docker and use the manual setup. Otherwise Docker is the easier ride.
...
### 🩺 Health and discovery URLs

| URL | What it tells you |
| :--- | :--- |
| `GET /health/live` | Process is alive. Always `200 ok` if it can answer at all. |
| `GET /health/ready` | DB is reachable. `200` if a `SELECT 1` succeeds; `503` if not. |
| `GET /health` | Alias for `/health/live` (backwards compatibility). |
| `GET /docs` | 📚 Interactive OpenAPI 3.1 spec with try-it-out UI. |
| `GET /docs/json` | Raw OpenAPI spec (JSON). |

---

## 🚀 4. Try it out / API Contracts

Five endpoints. Each one below is collapsed by default; click to see the contract and a curl example.
...
| **Method** | `POST` |
| **Path** | `/wallets` |
| **Idempotent** | No (rejects duplicate customer via `UNIQUE` constraint) |
...
**Error responses**: `400` (validation), `409` (customer already has a wallet).
...
| **Idempotent** | ✅ Yes, on `(walletId, referenceType, referenceId)` |

**Request body**

```jsonc
{
  "amount": "integer in minor units of the currency (1 to 2^53-1)",
  "currency": "USD" | "EUR" | "GBP" | "CAD" | "INR",
  "referenceType": "ORDER_SYSTEM | PAYMENT_SYSTEM | LOAN_SYSTEM | …",
  "referenceId": "the instruction ID from the source system"
}
```

**Response `201 Created`** (new entry) or **`200 OK`** (idempotent replay)
...
**Error responses**: `400` (validation), `404` (wallet not found), `422` (currency mismatch or amount overflow).

**Sample cURL**

```bash
curl -X POST http://localhost:8080/wallets/$WALLET/topup \
  -H 'content-type: application/json' \
  -d '{
    "amount": 50000,
    "currency": "USD",
...
```jsonc
{
  "amount": "integer in minor units (1 to 2^53-1)",
  "currency": "USD" | "EUR" | "GBP" | "CAD" | "INR",
  "referenceType": "ORDER_SYSTEM | …",
  "referenceId": "the instruction ID"
}
```

**Response `201 Created`** (new entry) or **`200 OK`** (idempotent replay)

Same shape as topup, with `entryType: "DEBIT"`.

**Error responses**: `400` (validation), `404` (wallet not found), `422` (insufficient balance, currency mismatch, or amount overflow).

**Sample cURL**

```bash
curl -X POST http://localhost:8080/wallets/$WALLET/deduct \
  -H 'content-type: application/json' \
  -d '{
    "amount": 10000,
    "currency": "USD",
...
**Error responses**: `400` (non-UUID path id), `404` (wallet not found).

**Sample cURL**

```bash
curl http://localhost:8080/wallets/$WALLET/balance
```

</details>

<details>
<summary><b>GET /wallets/:id/transactions</b> · The ledger (cursor-paginated)</summary>
...
| **Query** | `limit` (1-500, default 100), `cursor` (id of the last entry from the previous page) |
...
Iterate by feeding `nextCursor` back as `cursor` until `hasMore: false`.

**Sample cURL**

```bash
curl "http://localhost:8080/wallets/$WALLET/transactions?limit=50"
```
...
The interesting engineering choices. Each one is the answer to a hard question.
...
| | Reads | Writes |
| --- | --- | --- |
| Cost | O(n) per request | O(n), since every debit needs to check the balance before committing |
| Cacheable? | ✅ on reads | ❌ on writes (a stale balance during a debit allows double-spending) |

The write path is the killer. Caching hides the read cost but cannot hide the write cost. For an active wallet doing thousands of orders a day, this design buckles under its own success.
...
This is what production financial systems use (Stripe, Razorpay, banks). The chaos test asserts `SUM(signed amounts) == wallets.balance` after thousands of operations as a canary against the only way this can drift, which is a bug.
...
- A wallet that grows additional fields (status, tier, frozen-until) loses its natural home.
- "Show me all wallets with balance over $X" stops being a simple index scan.
...
**Decimals: integer minor units.** Every amount in the API is an integer in the smallest unit of the currency (cents for USD/EUR/GBP/CAD, paise for INR). This is the convention every major fintech API uses (Stripe, Razorpay, Adyen, Square, AWS Payments). Why?

- Integer math is exact. Floats accumulate rounding errors (`0.1 + 0.2 !== 0.3` in IEEE 754).
- JSON numbers carry numbers safely up to 2^53; staying as integers avoids precision loss in transit.
...
Two real limits live in our stack. Both are technical, not business.

| Layer | Limit | What happens when exceeded | Response |
| :--- | :--- | :--- | :--- |
| **API edge** (JSON schema) | `amount` ≤ Number.MAX_SAFE_INTEGER (2^53 − 1) | Above this, `JSON.parse` silently rounds | `400 VALIDATION_ERROR` |
| **Database** (BIGINT) | `balance + amount` ≤ 2^63 − 1 | Postgres raises SQLSTATE `22003` | `422 AMOUNT_OUT_OF_RANGE` |
...
Response carries `nextCursor` and `hasMore` so the caller knows when to stop.
