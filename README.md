...
A retry with the same `(referenceType, referenceId)` on the same wallet returns the prior result instead of running the operation again. See [Idempotency, in depth](#-idempotency-in-depth) for why the design works this way.
...
<details open>
<summary><b>📋 34 tests across 8 categories</b></summary>
...
| 🔁 Idempotency (debit) | Same reference returns same entry, balance unchanged |
...
| 💱 Currency safety | Mismatched currency rejected with 422; matching currency succeeds |
...
| 🛂 Strict input validation | 17 cases × 2 endpoints — negatives, zeros, floats, strings, null all rejected with 400 |
